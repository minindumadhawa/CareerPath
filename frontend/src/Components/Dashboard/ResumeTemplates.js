...
const TEMPLATES = [
  ...
  { id: 'modernTech',   name: 'Tech Forward',       category: 'modern',       premium: false, accent: '#0f766e', secondary: '#14b8a6', layout: 'sidebar-left',   previewLayout: 'sidebar',     description: 'Tech-focused clean layout with skills'   },
...
// Profile photo helper used inside previews
function ProfilePhoto({ src, size = 72, borderColor = '#ddd', style = {} }) {
  if (!src) return null;
  return (
    <img
      src={src}
      ...
      style={{
        ...
        border: `3px solid ${borderColor}`, display: 'block', flexShrink: 0,
        ...style,
      }}
    />
  );
...
// ═══════════════════════════════════════════════════════════════
// PREVIEW RENDERERS — all accept `photo` prop
// ═══════════════════════════════════════════════════════════════
function CorporatePreview({ d, t, photo }) {
  return (
    <div style={{ fontFamily: 'Georgia, serif' }}>
      <div style={{ background: t.accent, color: '#fff', padding: '28px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <ProfilePhoto src={photo} size={80} borderColor="#fff" />
...
function SidebarPreview({ d, t, photo }) {
  return (
    <div style={{ display: 'flex', fontFamily: 'Helvetica, Arial, sans-serif', minHeight: 600 }}>
      <div style={{ width: 240, background: t.accent, color: '#fff', padding: '28px 20px', flexShrink: 0 }}>
        {photo && <div style={{ marginBottom: 16 }}><ProfilePhoto src={photo} size={80} borderColor="rgba(255,255,255,0.3)" /></div>}
...
function EnterprisePreview({ d, t, photo }) {
  return (
    <div style={{ fontFamily: 'Georgia, serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${t.accent}, ${t.secondary})`, color: '#fff', padding: '32px', borderRadius: '0 0 12px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <ProfilePhoto src={photo} size={80} borderColor="rgba(255,255,255,0.4)" />
          <div>
...
function ExecutivePreview({ d, t, photo }) {
  return (
    <div style={{ display: 'flex', fontFamily: '"Palatino Linotype", Georgia, serif', minHeight: 600 }}>
      <div style={{ width: 260, background: t.accent, color: '#fff', padding: '32px 22px', flexShrink: 0 }}>
        {photo && <div style={{ marginBottom: 16 }}><ProfilePhoto src={photo} size={80} borderColor={t.secondary} /></div>}
...
function MinimalPreview({ d, photo }) {
  return (
    <div style={{ fontFamily: '"Helvetica Neue", Helvetica, sans-serif', padding: '40px 44px', lineHeight: 1.7, maxWidth: 680, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, marginBottom: 8 }}>
        ...
        <div style={{ textAlign: photo ? 'left' : 'center' }}>
          <h1 style={{ fontSize: '2.6rem', fontWeight: 300, margin: 0, letterSpacing: 3 }}>{d.fullName}</h1>
          <div style={{ fontSize: '0.88rem', color: '#999', marginTop: 4, letterSpacing: 1 }}>{contactLine(d)}</div>
        </div>
      </div>
      {d.summary && <><div style={{ height: 1, background: '#eee', margin: '16px 0' }} /><p style={{ margin: '0 0 24px', textAlign: 'center', fontStyle: 'italic', color: '#555' }}>{d.summary}</p></>}
...
function CreativePreview({ d, t, photo }) {
  return (
...
function TerminalPreview({ d, t, photo }) {
  return (
    <div style={{ background: t.accent, color: t.secondary, fontFamily: '"Courier New", monospace', padding: '24px 28px', minHeight: 500, borderRadius: 8 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
        <ProfilePhoto src={photo} size={60} borderColor={`${t.secondary}40`} />
        <div>
          <div style={{ opacity: 0.5, fontSize: '0.8rem' }}>$ cat resume.json</div>
          <h1 style={{ color: '#fff', fontSize: '1.8rem', margin: '4px 0' }}>{d.fullName}</h1>
          <div style={{ fontSize: '0.85rem', opacity: 0.5 }}>{contactLine(d)}</div>
        </div>
      </div>
...
function TwoColumnPreview({ d, t, photo }) {
  return (
    <div style={{ fontFamily: 'Arial, sans-serif' }}>
      <div style={{ background: t.accent, color: '#fff', padding: '24px 30px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <ProfilePhoto src={photo} size={72} borderColor="rgba(255,255,255,0.4)" />
          <div>
...
function StartupPreview({ d, t, photo }) {
  return (
    <div style={{ fontFamily: '"Segoe UI", sans-serif', padding: '20px' }}>
      <div style={{ background: `${t.accent}0a`, border: `2px solid ${t.accent}20`, borderRadius: 12, padding: '24px 28px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <ProfilePhoto src={photo} size={72} borderColor={`${t.accent}30`} />
          <div>
            <h1 style={{ fontSize: '2rem', margin: 0, color: t.accent }}>{d.fullName}</h1>
            <div style={{ marginTop: 4, fontSize: '0.88rem', color: '#888' }}>{contactLine(d)}</div>
            {d.summary && <p style={{ margin: '8px 0 0', lineHeight: 1.6, color: '#555', fontSize: '0.9rem' }}>{d.summary}</p>}
          </div>
        </div>
      </div>
...
// ─── Preview Router ────────────────────────────────────────────
function PreviewContent({ data, templateId, photo }) {
  const tmpl  = TEMPLATES.find(t => t.id === templateId) || TEMPLATES[0];
  const pl    = tmpl.previewLayout;
  const props = { d: data, t: tmpl, photo };
  switch (pl) {
...
function ResumeTemplates() {
  ...
  // ── Profile photo state ──────────────────────────────────
  // Priority: 1) manually changed here  2) from API profilePicture  3) null
  ...
  useEffect(() => {
    ...
    // Restore manually uploaded photo from a previous session
    try {
      const saved = localStorage.getItem('rtManualPhoto');
      if (saved) { setProfilePhoto(saved); setPhotoSource('manual'); }
    } catch {}
  }, []);
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { setInfoMessage('Photo must be under 2 MB.'); return; }
    const reader = new FileReader();
    reader.onload = (ev) => {
      const dataUrl = ev.target.result;
      setProfilePhoto(dataUrl);
      setPhotoSource('manual');
      try { localStorage.setItem('rtManualPhoto', dataUrl); } catch {}
      // Update previewData with new photo
      if (previewData) setPreviewData(prev => prev ? { ...prev, profilePicture: dataUrl } : prev);
    };
    reader.readAsDataURL(file);
  };

  const handleRemovePhoto = () => {
    try { localStorage.removeItem('rtManualPhoto'); } catch {}
    if (photoInputRef.current) photoInputRef.current.value = '';
    const apiPhoto = previewData?.profilePicture && photoSource === 'manual' ? null : null;
    // Revert to API photo from previewData if available
    if (previewData?._apiProfilePicture) {
      setProfilePhoto(previewData._apiProfilePicture);
      setPhotoSource('api');
    } else {
  ...
  const onPreview = useCallback(async () => {
    ...
    try {
      ...
      // Store original API photo for revert
      data._apiProfilePicture = data.profilePicture;
      setPreviewData(data);
      setPreviewMode(true);

      // Set photo from API only if user hasn't manually set one
      if (data.profilePicture && photoSource !== 'manual') {
        setProfilePhoto(data.profilePicture);
        setPhotoSource('api');
      }
    } catch (e) {
      setInfoMessage('Error: ' + (e.message || 'Unknown'));
    } finally {
      setIsLoading(false);
    }
  }, [selectedTemplate, profileId, photoSource]);
  ...
  const onDownload = useCallback(async () => {
    ...
    try {
      ...
      if (!previewMode) {
        profile._apiProfilePicture = profile.profilePicture;
        setPreviewData(profile);
        setPreviewMode(true);
        if (profile.profilePicture && photoSource !== 'manual') { setProfilePhoto(profile.profilePicture); setPhotoSource('api'); }
        await new Promise(resolve => setTimeout(resolve, 600));
      }

      let targetElement = document.getElementById('cv-document');
      if (!targetElement) throw new Error('Could not find CV document. Please click Preview first, then Download.');

      const orig = { width: targetElement.style.width, maxWidth: targetElement.style.maxWidth, boxShadow: targetElement.style.boxShadow, borderRadius: targetElement.style.borderRadius };
      ...
      const canvas = await html2canvas(targetElement, {
        scale: 2, logging: false, useCORS: true, backgroundColor: '#ffffff', width: 800, windowWidth: 800,
        onclone: (clonedDoc) => {
          const el = clonedDoc.getElementById('cv-document');
          if (el) { el.style.width = '800px'; el.style.maxWidth = '800px'; el.style.boxShadow = 'none'; el.style.borderRadius = '0'; }
        },
      });
      Object.assign(targetElement.style, orig);
      ...
      const pgCanvasH = (pgH / imgH) * canvas.height;
      let yOff = 0, pgNum = 0;

      while (yOff < canvas.height) {
        if (pgNum > 0) pdf.addPage();
        const sliceH = Math.min(pgCanvasH, canvas.height - yOff);
        ...
        pdf.addImage(pc.toDataURL('image/png', 1.0), 'PNG', 0, 0, imgW, (sliceH * imgW) / canvas.width);
        yOff += pgCanvasH;
        pgNum++;
      }
      ...
      setInfoMessage('PDF error: ' + (err.message || 'Unknown. Try Preview first, then Download.'));
    } finally {
      const ov = document.getElementById('pdf-overlay');
      if (ov && document.body.contains(ov)) document.body.removeChild(ov);
      setPdfGenerating(false);
    }
  }, [selectedTemplate, profileId, previewMode, photoSource]);

  const sel = TEMPLATES.find(t => t.id === selectedTemplate);

  // The photo passed to previews: manual override wins, else API, else null
  const activePhoto = profilePhoto;
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '40px 32px 0' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button className="rt-b" onClick={() => photoInputRef.current?.click()} style={{ padding: '9px 16px', background: '#fff', color: '#1c1c1c', borderRadius: 9, fontWeight: 600, fontSize: '0.84rem', border: '1.5px solid #ddd', display: 'flex', alignItems: 'center', gap: 6 }}>
              {activePhoto
                ? <><img src={activePhoto} alt="" style={{ width: 22, height: 22, borderRadius: '50%', objectFit: 'cover' }} /> {photoSource === 'api' ? 'Change Photo' : 'Change Photo'}</>
                : '📸 Add Photo'}
            </button>
            {activePhoto && (
              <button className="rt-b" onClick={handleRemovePhoto} style={{ padding: '9px 14px', background: '#fff1f2', color: '#e11d48', borderRadius: 9, fontWeight: 600, fontSize: '0.84rem', border: '1.5px solid #fda4af' }}>
                ✕ Remove
              </button>
            )}
            {/* Search */}
        ...
        {/* Photo status banner */}
        {activePhoto && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: 9, marginBottom: 16, fontSize: '0.85rem', color: '#065f46', fontWeight: 600 }}>
            <img src={activePhoto} alt="" style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', border: '2px solid #6ee7b7' }} />
            {photoSource === 'api'    ? '✓ Profile photo loaded from your Student Profile — will appear in CV.' : '✓ Custom photo uploaded — will appear in CV.'}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 28, overflowX: 'auto' }}>
          {CATEGORIES.map(cat => { const a = activeCategory === cat.id; return <button key={cat.id} className="rt-cat" onClick={() => setActiveCategory(cat.id)} style={{ padding: '8px 16px', fontSize: '0.84rem', fontWeight: a ? 700 : 500, fontFamily: 'inherit', borderRadius: 8, background: a ? '#1c1c1c' : '#edebe7', color: a ? '#fff' : '#5a5a52', display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 11 }}>{cat.icon}</span>{cat.label}</button>; })}
        </div>
      </div>
        ...
        <div style={{ maxWidth: 1240, margin: '0 auto 20px', padding: '0 32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: '#1c1c1c', borderRadius: 12, color: '#fff', flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              {activePhoto && <img src={activePhoto} alt="" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.3)' }} />}
              <div style={{ width: activePhoto ? 0 : 36, height: activePhoto ? 0 : 36, overflow: 'hidden', borderRadius: 8, background: sel.accent, display: activePhoto ? 'none' : 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#fff', fontWeight: 800 }}>✓</div>
          ...
          <div style={{ padding: '2rem', background: '#fff', borderRadius: 16, border: '1px solid #e4e4e7', boxShadow: '0 8px 30px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '0.85rem', color: '#9a9a92' }}>
                  {activePhoto ? (photoSource === 'api' ? '📸 Photo from Student Profile included' : '📸 Custom photo included') : 'No photo — add one in Student Profile or using the button above'}
                </p>
              </div>
            ...
            <div id="cv-document" ref={pdfRenderRef} style={{ background: '#fff', maxWidth: 900, margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', borderRadius: 8, overflow: 'hidden' }}>
              <PreviewContent data={previewData} templateId={selectedTemplate} photo={activePhoto} />
            </div>
